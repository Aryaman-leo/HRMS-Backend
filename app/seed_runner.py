"""Seed DB with departments, employees, and sample attendance. Idempotent; skips if data exists."""
import copy
...
def run_seed():
    ...
    try:
        ...
        for data in SEED_EMPLOYEES:
            row = copy.copy(data)
            dept_name = row.pop("department_name")
            ...
            if dept_id is None:
                ...
                if dept:
                    ...
                    db.flush()
                    dept_id = dept.id
            db.add(Employee(**row, department_id=dept_id))
        db.commit()
